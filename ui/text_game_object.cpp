...
TextGameObject::TextGameObject(const glm::vec3 &position, Geometry *geom, Shader *shader, GLuint texture) : GameObject(position, geom, shader, texture, 0, false) {
...
void TextGameObject::Render(glm::mat4 view_matrix, double current_time) {
    ...
    if(parent_) {
        translation_matrix = glm::translate(glm::mat4(1.0f), position_ + parent_->GetPosition());
    }
    else {
